...
def make_line_plot_ingestion_time(ostrich_l, ostrich_opt_l, cobra_pre_l, cobra_l):
    ...
    plt.plot(ostrich_l[0], map(get_s, ostrich_l[4]), '-', color ="red", label='OSTRICH-1F',markevery=20)
    plt.plot(ostrich_opt_l[0], map(get_s, ostrich_opt_l[4]), '-', color = "orange", label='OSTRICH-2F',markevery=20)
    plt.plot(cobra_pre_l[0], map(get_s, cobra_pre_l[4]), '-', color = "royalblue", label='COBRA-PRE_FIX_UP',markevery=20)
    plt.plot(cobra_pre_l[0], map(get_s, cobra_l[4]), '-', color ="b", label='COBRA-OUT_OF_ORDER',markevery=20)
...
def make_line_plot_total_ingestion_time(ostrich_l, ostrich_opt_l, cobra_pre_l, cobra_l):
    fig = plt.figure(figsize=(12,12))
    plt.plot(ostrich_l[0], map(get_min, ostrich_l[2]), '.-', color ="red", label='OSTRICH-1F',markevery=20)
    plt.plot(ostrich_opt_l[0], map(get_min, ostrich_opt_l[2]), '>-', color = "orange", label='OSTRICH-2F',markevery=20)
    plt.plot(cobra_pre_l[0], map(get_min, cobra_pre_l[2]), 'x-', color = "royalblue", label='COBRA-PRE_FIX_UP',markevery=20)
    plt.plot(cobra_pre_l[0], map(get_min, cobra_l[2]), '^-', color ="b", label='COBRA-OUT_OF_ORDER',markevery=20)
...
def make_line_plot_storage_size(ostrich_l, ostrich_opt_l, cobra_pre_l, cobra_l):
    ...
    fig = plt.figure(figsize=(12,12))
    plt.plot(ostrich_l[0], map(get_MB, ostrich_l[3]), '.-', color ='r', label='OSTRICH-1F',markevery=20)
    plt.plot(ostrich_opt_l[0], map(get_MB, ostrich_opt_l[3]), '>-', color = 'orange', label='OSTRICH-2F',markevery=20)
    plt.plot(cobra_pre_l[0], map(get_MB, cobra_pre_l[3]), 'x-', color = 'royalblue', label='COBRA-PRE_FIX_UP',markevery=20)
    plt.plot(cobra_pre_l[0], map(get_MB, cobra_l[3]), '^-', color ='b', label='COBRA-OUT_OF_ORDER',markevery=20)
    plt.ylabel('Size (MB)')
    plt.xlabel('Version')
    art = []
    lgd = plt.legend(loc=9, bbox_to_anchor=(0.40, -0.2), ncol=2)
...
def make_line_plot_added(ostrich_l, ostrich_opt_l, cobra_pre_l, cobra_l):
    ...
    fig = plt.figure(figsize=(12,12))
    plt.plot(ostrich_l[0], map(get_K, ostrich_l[1]), '.-', color ='r', label='OSTRICH-1F',markevery=20)
    plt.plot(ostrich_opt_l[0],  map(get_K, ostrich_opt_l[1]), '>-', color = 'orange', label='OSTRICH-2F',markevery=20)
    plt.plot(cobra_pre_l[0],  map(get_K, cobra_pre_l[1]), 'x-', color = 'royalblue', label='COBRA-PRE_FIX_UP',markevery=20)
    plt.plot(cobra_pre_l[0],  map(get_K, cobra_l[1]), '^-', color ='b', label='COBRA-OUT_OF_ORDER',markevery=20)
