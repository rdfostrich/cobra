...
def make_line_plot_total_ingestion_time(ostrich_l, ostrich_opt_l, cobra_pre_l, cobra_l):
    ...
    plt.plot(ostrich_l[0], map(get_hour, ostrich_l[2]), '.-', color ="red", label='OSTRICH-1F')
    plt.plot(ostrich_opt_l[0], map(get_hour, ostrich_opt_l[2]), '>-', color = "orange", label='OSTRICH-2F')
    plt.plot(cobra_pre_l[0], map(get_hour, cobra_pre_l[2]), 'x-', color = "royalblue", label='COBRA-PRE_FIX_UP')
    plt.plot(cobra_pre_l[0], map(get_hour, cobra_l[2]), '^-', color ="b", label='COBRA-OUT_OF_ORDER')
...
def make_line_plot_ingestion_time(ostrich_l, ostrich_opt_l, cobra_pre_l, cobra_l):
    ...
    fig = plt.figure(figsize=(12,12))
    plt.plot(ostrich_l[0], map(get_min, ostrich_l[4]), '.-', color ="red", label='OSTRICH-1F')
    plt.plot(ostrich_opt_l[0], map(get_min, ostrich_opt_l[4]), '>-', color = "orange", label='OSTRICH-2F')
    plt.plot(cobra_pre_l[0], map(get_min, cobra_pre_l[4]), 'x-', color = "royalblue", label='COBRA-PRE_FIX_UP')
    plt.plot(cobra_pre_l[0], map(get_min, cobra_l[4]), '^-', color ="b", label='COBRA-OUT_OF_ORDER')
...
def make_line_plot_storage_size(ostrich_l, ostrich_opt_l, cobra_pre_l, cobra_l):
    ...
    fig = plt.figure(figsize=(12,12))
    plt.plot(ostrich_l[0], map(get_GB, ostrich_l[3]), '.-', color ='r', label='OSTRICH-1F')
    plt.plot(ostrich_opt_l[0], map(get_GB, ostrich_opt_l[3]), '>-', color = 'orange', label='OSTRICH-2F')
    plt.plot(cobra_pre_l[0], map(get_GB, cobra_pre_l[3]), 'x-', color = 'royalblue', label='COBRA-PRE_FIX_UP')
    plt.plot(cobra_pre_l[0], map(get_GB, cobra_l[3]), '^-', color ='b', label='COBRA-OUT_OF_ORDER')
    plt.ylabel('Size (GB)')
    plt.xlabel('Version')
    art = []
    lgd = plt.legend(loc=9, bbox_to_anchor=(0.40, -0.2), ncol=2)
...
def make_line_plot_added(ostrich_l, ostrich_opt_l, cobra_pre_l, cobra_l):
    ...
    fig = plt.figure(figsize=(12,12))
    plt.plot(ostrich_l[0], map(get_mil, ostrich_l[1]), '.-', color ='r', label='OSTRICH-1F')
    plt.plot(ostrich_opt_l[0],  map(get_mil, ostrich_opt_l[1]), '>-', color = 'orange', label='OSTRICH-2F')
    plt.plot(cobra_pre_l[0],  map(get_mil, cobra_pre_l[1]), 'x-', color = 'royalblue', label='COBRA-PRE_FIX_UP')
    plt.plot(cobra_pre_l[0],  map(get_mil, cobra_l[1]), '^-', color ='b', label='COBRA-OUT_OF_ORDER')
    plt.ylabel('Processed Triples (M)')
    plt.xlabel('Version')
    art = []
    lgd = plt.legend(loc=9, bbox_to_anchor=(0.40, -0.2), ncol=2)
    art.append(lgd)
...
#     ax.plot(np.arange(len(cobra_l[0])), map(get_hour, cobra_l[2]), 'xb-', label='COBRA')
...
#     fig = plt.figure()
#     ax = fig.add_subplot(111)
#     ax.plot(map(get_GB, cobra_l[3]), 'xb-', label='COBRA')
...
def make_table(ostrich_tup, ostrich_mod_tup, pre_fix_up_cobra_tup, post_fix_up_cobra_tup, cobra_tup):
    ostrich_dur = str(get_hour(ostrich_tup[2][-1]))
    ostrich_mod_dur = str(get_hour(ostrich_mod_tup[2][-1]))
    pre_fix_up_cobra_dur = str(get_hour(pre_fix_up_cobra_tup[2][-1]))
    post_fix_up_cobra_dur = str(get_hour(post_fix_up_cobra_tup[2][-1]))
    cobra_dur = str(get_hour(cobra_tup[2][-1]))

    ostrich_size = str(get_GB(ostrich_tup[3][-1]))
    ostrich_mod_size = str(get_GB(ostrich_mod_tup[3][-1]))
    pre_fix_up_cobra_size = str(get_GB(pre_fix_up_cobra_tup[3][-1]))
    post_fix_up_cobra_size = str(get_GB(post_fix_up_cobra_tup[3][-1]))
    cobra_size =  str(get_GB(cobra_tup[3][-1]))

    print("Storage Layout & Storage Size (GB) & Ingestion Time (hour) \\ \hline")
...
print(ostrich_tup[0])
print(ostrich_mod_tup[0])
print(cobra_tup[0])

print(ostrich_tup[2])
print(ostrich_mod_tup[2])
print(cobra_tup[2])
