#!/bin/bash
# A utility to join files in a directory that have a '*.????.*' pattern in their name.
# The joint output is in a file named '*.*', i.e. the '.????' portion is deleted
#
# Parameters
...
find ${INDIR} -maxdepth 1 -type f -iname '*.????.*' | sort | while read f ; do
  echo $f
  base=${f##*/}
