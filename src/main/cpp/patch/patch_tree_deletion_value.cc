...
string PatchTreeDeletionValueElement::to_string() const {
    string res = "" + PatchTreeDeletionValueElementBase::to_string();
    if(local_change)
        res+= "L";
    return res + ":" + get_patch_positions().to_string();
}
...
template <class T>
const T& PatchTreeDeletionValueBase<T>::get_last_patch() const {
...
template <class T>
bool PatchTreeDeletionValueBase<T>::is_reverse_local_change(int patch_id) const {
    if (get_size() == 0) return false;
    T item(patch_id);
    typename std::vector<T>::const_iterator findIt = std::upper_bound(elements.begin(), elements.end(), item);
    if(findIt < elements.begin()) {
        throw std::runtime_error("Tried to retrieve a patch_id that was too low. (PatchTreeDeletionValue::is_local_change),"
                                         "tried to find " + std::to_string(patch_id) + " in " + this->to_string());
    }
...
template <class T>
void PatchTreeDeletionValueBase<T>::remove_versions(const int version) {
    int original_size = elements.size();

    // erase versions
    elements.erase(std::remove_if(
            elements.begin(), elements.end(),
            [version](T t) {
                return t.get_patch_id() >= version;
            }), elements.end());
}

template<class T>
const vector<T> &PatchTreeDeletionValueBase<T>::getElements() const {
    return elements;
}
