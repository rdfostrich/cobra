...
//int PatchTreeAdditionValue::get_last_patch_id() const {
//    return i < get_size() ? patches.back() : -1;
//}
...
bool PatchTreeAdditionValue::is_local_change(int patch_id) const { // 1 3L 5
...
bool PatchTreeAdditionValue::is_reverse_local_change(int patch_id) const { // find just higher  1 3L 5
    if (local_changes.size() == 0) return false;
    std::vector<int>::const_iterator findIt = std::upper_bound(local_changes.begin(), local_changes.end(), patch_id);
    if(findIt >= local_changes.end()) findIt--;
    return *findIt >= patch_id;
}

//bool PatchTreeAdditionValue::is_reverse_local_change(int patch_id) const {
//    if (local_changes.size() == 0) return false;
//    std::vector<int>::const_iterator findIt = std::lower_bound(local_changes.begin(), local_changes.end(), patch_id);
//    if(findIt >= local_changes.end()) findIt--;
//    return *findIt >= patch_id;
...
const std::vector<int> &PatchTreeAdditionValue::getPatches() const{
    return patches;
}

const std::vector<int> &PatchTreeAdditionValue::getLocal_changes() const {
    return local_changes;
}

void PatchTreeAdditionValue::remove_versions(const int version) {
    // erase versions
    ...
    // erase local changes
    local_changes.erase(std::remove_if(
            local_changes.begin(), local_changes.end(),
            [version](const int& x) {
                return x >= version;
            }), local_changes.end());
}
