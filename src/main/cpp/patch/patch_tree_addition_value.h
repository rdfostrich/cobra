...
public:
    ...
    const std::vector<int> &getPatches() const;

    const std::vector<int> &getLocal_changes() const;

    /**
     * removes versions if version >=version
     * @param version
     */
    void remove_versions(int version);

//    int get_last_patch_id() const;

    bool is_reverse_local_change(int patch_id) const;
};
