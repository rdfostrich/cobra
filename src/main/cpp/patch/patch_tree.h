...
#define TEMP_PATCHTREE_FILENAME_BASE(id) ("temp_patchtree_" + std::to_string(id) + ".kct")
#define TEMP_PATCHTREE_FILENAME(id,suffix) (TEMP_PATCHTREE_FILENAME_BASE(id) + "_" + suffix)
#define TEMP_METADATA_FILENAME_BASE(id) ("temp_meta_" + std::to_string(id) + ".dat")
...
public:
     ...
     * Append the given patch elements to the tree with given patch id.
     * This can OVERWRITE existing elements without a warning.
     * Only works if patch_ids are inserted in decreasing order (e.g. first 5 than 4 ...)
     * @param patch_it A patch iterator with sorted (SPO) elements
     * @param patch_id The id of the patch
     * @param progressListener an optional progress listener.
     * @note If an error occurs, some elements might have already been added.
     * If you want to change this behaviour, you'll have to first check if the patch elements are really new.
     */
    void reverse_append_unsafe(PatchElementIterator *patch_it, int patch_id, ProgressListener *progressListener = NULL);
    /**
     * Append the given patch elements to the tree with given patch id.
    ...
    bool reverse_append(PatchElementIterator *patch_it, int patch_id, ProgressListener *progressListener);
    ...
    bool reverse_append(const PatchSorted &patch, int patch_id, ProgressListener *progressListener);
    /**
    ...
    /**
     * purges all versions >= version
     * @param version
     */
    void purge_versions(int version, DictionaryManager* dict);

    TripleStore *getTripleStore() const;
