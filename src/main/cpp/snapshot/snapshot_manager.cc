...
void SnapshotManager::load_all_snapshots(){
    for(std::map<int,HDT*>::iterator iter = loaded_snapshots.begin(); iter != loaded_snapshots.end(); ++iter) {
        load_snapshot(iter->first);
    }
}
...
void SnapshotManager::remove_snapshot(int snapshot_id) {
    //remove snapshot
    if(loaded_snapshots[snapshot_id] != NULL){
        delete loaded_snapshots[snapshot_id];
    }
    loaded_snapshots.erase(snapshot_id);

    //remove dictionnary
    if(loaded_dictionaries[snapshot_id] != NULL){
        delete loaded_dictionaries[snapshot_id];
    }
    loaded_dictionaries.erase(snapshot_id);
}
