...
int main(int argc, char** argv) {
    ...
    int patch_id_start = std::atoi(argv[2]);
    int patch_id_end = std::atoi(argv[3]);
    int offset = argc == 8 ? std::atoi(argv[7]) : 0;

    // Construct query
    DictionaryManager* dict = controller.get_dictionary_manager(patch_id_start);
